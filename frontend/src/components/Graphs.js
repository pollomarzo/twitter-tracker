...
import {AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Radar, RadarChart, PolarGrid, PolarAngleAxis, PieChart, Pie, Cell, Legend} from 'recharts';
...
const Graphs = ({ list }) => {
  ...
  useEffect(() => {
    ...
    list.forEach((listElement) => {
      ...
      if ((listElement.coordinates && listElement.coordinates.type === 'Point') || (listElement.place && listElement.place.bounding_box)){
        geoObject[0].count += 1;
      } else {
        geoObject[1].count += 1;
      }
      if(listElement.place != null){
        ...
        regionName.push((listElement.place.full_name).split(",")[1] + "-" + listElement.place.country_code)
      } else {
